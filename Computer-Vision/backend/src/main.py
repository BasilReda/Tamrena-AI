...
def main():
    ...
    # Parse CLI flags: 'c' for webcam, 's' for saving output, 'time' to force validation timer
    ...
    if use_webcam_flag:
        ...
        remaining_args = [arg for arg in args[1:] if arg.lower() not in ("s", "c", "time")]
        video_path = remaining_args[0] if remaining_args else None

    force_timer_flag = "time" in lower_args
    settings.FORCE_TIMER_VALIDATION = force_timer_flag
