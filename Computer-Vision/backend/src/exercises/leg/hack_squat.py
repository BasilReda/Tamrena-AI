    ...
    counter_rules: list[AngleCounterRule] = field(
        default_factory=lambda: [
            AngleCounterRule(
                ...
                up_angle=120,                   # crossing back above -> RETURNING phase
                down_angle=90,                  # <= 90° -> DOWN phase begins (rep window starts)
                min_rom_angle=80,               # GOOD rep must reach <= 85° at the bottom
                max_rom_angle=135,              # ... and >= 150° at full extension before reversing
            ),
        ]
    )
