...
class DisplaySettings:
    ...
    show_validation_skeleton: bool = True
    show_angle_labels: bool = True      # <-- ADD: set False to hide "N deg" boxes
    show_center_line: bool = False      # <-- ADD: draws shoulder-mid -> hip-mid guide line
    segment_lines: list[SegmentLine] = field(default_factory=list)
...
class Exercise:
    ...
    * Lists (``counter_rules`` / ``validation_rules``) are the reason the design
    ...
    # Per-exercise 3D override: True = force 3D, False = force 2D, None = use .env USE_3D.
    # By default all exercises default to 3D (True).
