...
class RepResult:
    ...
    Attributes:
        number:       1-based index of the repetition (matches the on-screen
                      rep counter).
        good:         ``True`` iff the repetition had no ``error``- or
...
class RepJudge:
    ...
    def finalize_rep(self, rep_number: int, frame: int = 0, force_good: bool | None = None) -> RepResult:
        ...
        Good/Bad classification rule (v1 behaviour restored):
        - ERROR severity   → BAD (dangerous, invalid rep)
        - WARNING severity → BAD (form issue must be corrected)
        - INFO severity    → GOOD (informational only, score unaffected)
        """
        if force_good is not None:
            good = force_good
        else:
            # ERROR and WARNING both mark the rep BAD (v1 behaviour).
            # INFO is informational only and does not affect quality.
            bad = any(
                v.severity in (Severity.ERROR, Severity.WARNING)
