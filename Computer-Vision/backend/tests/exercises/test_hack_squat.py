...
def main():
    ...
    assert (counter.up_angle, counter.down_angle) == (120, 90)   # user-calibrated
    # managed ROM path: extremes set, tempo gate off
    assert (counter.min_rom_angle, counter.max_rom_angle) == (80, 135)
    ...
    assert (guard.min_angle, guard.max_angle) == (60, 165)
    ...
    for visible in ("left", "right"):
        engine = run_session(visible, reps=[(78, 152), (78, 152)])
        ...
        engine2 = run_session(visible, reps=[(78, 152), (78, 172)])
    ...
    engine3 = run_session("left", reps=[(78, 152), (78, 172)])
