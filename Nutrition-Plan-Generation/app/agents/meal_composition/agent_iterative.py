"""
Iterative Meal Composition Agent — Free-form Arabic mode (llm_arabic).
...
- Generates authentic Egyptian meals in Arabic.
...
from app.agents.meal_composition.prompt_slot import (
    SYSTEM_PROMPT_SLOT,
...
async def _generate_slot_options(
    slot: MealSlotTarget,
    ...
    """
    user_msg = USER_PROMPT_SLOT_TEMPLATE.format(
    ...
    messages = [
        SystemMessage(content=SYSTEM_PROMPT_SLOT),
    ...
    for attempt in range(1, 4):
        try:
            logger.info(
                "Slot=%s attempt=%d — calling LLM (1 call for 3 options)",
                slot.slot_key,
                attempt,
            )
            ...
            logger.info(
                "Slot=%s complete | A=%.0f kcal  B=%.0f kcal  C=%.0f kcal",
                slot.slot_key,
    ...
    raise ValueError(
        f"Failed to generate meal options for slot={slot.slot_key} after 3 attempts: {last_error}"
    ) from last_error
...
async def run_meal_composition_iterative(
    macro_result: MacroResult,
    meal_distribution: MealDistribution,
) -> TripleMealPlan:
    """
    ...
    for slot_key in slot_order:
        ...
        meal_a, meal_b, meal_c = await _generate_slot_options(slot, macro_result)
    ...
    logger.info(
        "TripleMealPlan complete (4 LLM calls) | A=%.0f kcal  B=%.0f kcal  C=%.0f kcal",
