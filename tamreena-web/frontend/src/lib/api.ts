// Empty by default: the production nginx build proxies /auth, /api and /ws
// to the backend at the same origin, so relative paths just work. Set
// VITE_API_BASE_URL only for setups without that proxy (e.g. local dev
// against the Vite dev server, see docker-compose.yml).
 ...
 * that explicitly indicates session invalidation ('invalid_session'),
 * clears it and hard-redirects to /signin.
 */
...
export type LiveSessionTarget =
  | { mode: 'video'; videoId: string }
  | { mode: 'camera' }
  | string;

export function getLiveSessionWebSocketUrl(
  exerciseId: string,
  target: LiveSessionTarget,
): string {
  const token = getToken();
  // WebSocket requires an absolute ws(s):// URL — unlike fetch/EventSource,
  // it won't resolve a relative path against the page origin. When
  // API_BASE_URL is empty (same-origin nginx proxy) we build one explicitly
  // from window.location instead.
  const wsBase = API_BASE_URL
    ? API_BASE_URL.replace(/^http/, 'ws')
    : `${window.location.protocol === 'https:' ? 'wss:' : 'ws:'}//${window.location.host}`;

  let query = `exercise=${encodeURIComponent(exerciseId)}&token=${encodeURIComponent(token ?? '')}`;

  if (typeof target === 'string') {
    query += `&source=video&video=${encodeURIComponent(target)}`;
  } else if (target.mode === 'camera') {
    query += `&source=browser`;
  } else {
    query += `&source=video&video=${encodeURIComponent(target.videoId)}`;
  }

  return `${wsBase}/ws/live-session?${query}`;
}
...
export async function getLastNutritionRunId(): Promise<string | null> {
  const res = await authFetch('/api/nutrition/last');
  if (!res.ok) throw new Error(await parseErrorMessage(res, `Failed to load last nutrition run (${res.status})`));
  const body = await res.json();
  return body.run_id;
}
