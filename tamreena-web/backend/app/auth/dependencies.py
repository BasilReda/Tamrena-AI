...
def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(_bearer_scheme)) -> dict:
    ...
    if user is None:
        raise HTTPException(401, {"error": "invalid_session", "detail": "User no longer exists."})
    return user
...
def get_verified_token_for_stream(
    ...
    if not raw_token:
        raise HTTPException(401, {"error": "invalid_session", "detail": "Missing authentication token."})
    try:
