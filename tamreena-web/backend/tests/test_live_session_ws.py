...
@pytest.fixture(autouse=True)
def _own_test_upload(dynamo_table):
    """Most tests below drive the proxy with video=abc123 as the test
    user — record that ownership upfront so the new owns() gate the proxy
    applies for source=video doesn't reject them."""
    record_ownership("abc123", "507f1f77bcf86cd799439011", "upload")
...
def test_live_session_proxy_relays_state_event_and_binary_frame(monkeypatch):
    ...
    monkeypatch.setattr(routes.websockets, "connect", lambda uri, **kwargs: _FakeUpstreamConnection(canned, sent))
...
def test_live_session_proxy_records_ownership_of_cv_session_id_from_end_event(monkeypatch):
    from app.live_session.ownership import get_owner

    sent = []
    canned = [json.dumps({"type": "end", "reps": 3, "good": 2, "bad": 1, "session_id": "cv-sess-1"})]
    monkeypatch.setattr(routes.websockets, "connect", lambda uri, **kwargs: _FakeUpstreamConnection(canned, sent))
    ...
    assert get_owner("cv-sess-1") == "507f1f77bcf86cd799439011"
...
def test_live_session_proxy_forwards_stop_command_upstream(monkeypatch):
    sent = []
    monkeypatch.setattr(routes.websockets, "connect", lambda uri, **kwargs: _FakeUpstreamConnection([], sent))
...
def test_live_session_proxy_url_encodes_exercise_and_video_params(monkeypatch):
    record_ownership("x&source=webcam", "507f1f77bcf86cd799439011", "upload")
    captured_uri = {}
...
def test_live_session_proxy_rejects_upload_owned_by_another_user(monkeypatch):
    record_ownership("someone-elses-upload", "another-user-id", "upload")
    monkeypatch.setattr(routes.websockets, "connect", lambda uri, **kwargs: (_ for _ in ()).throw(AssertionError("should not connect upstream")))

    client = _client()
    with client.websocket_connect(
        f"/ws/live-session?exercise=biceps_curl&video=someone-elses-upload&token={_token()}"
    ) as ws:
        message = ws.receive_json()
        assert message["type"] == "error"


def test_live_session_proxy_rejects_upload_with_no_ownership_record(monkeypatch):
    monkeypatch.setattr(routes.websockets, "connect", lambda uri, **kwargs: (_ for _ in ()).throw(AssertionError("should not connect upstream")))

    client = _client()
    with client.websocket_connect(
        f"/ws/live-session?exercise=biceps_curl&video=never-uploaded&token={_token()}"
    ) as ws:
        message = ws.receive_json()
        assert message["type"] == "error"
