...
def test_stream_returns_json_error_not_sse_when_upstream_errors():
    from app.auth import models
    models.set_last_nutrition_run_id("507f1f77bcf86cd799439011", "unknown")
...
@respx.mock
def test_stream_rejects_run_id_belonging_to_another_user():
    from app.auth import models
...
@respx.mock
def test_stream_translates_upstream_401_to_503():
    from app.auth import models
    models.set_last_nutrition_run_id("507f1f77bcf86cd799439011", "abc123")

    respx.get(f"{NUTRITION_API_URL}/api/v1/nutrition/stream/abc123").mock(
