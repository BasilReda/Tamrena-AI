...
from app.auth.dependencies import get_current_user, get_verified_token, get_verified_token_for_stream
...
def _test_app() -> FastAPI:
    ...
    @app.get("/needs-user")
    def needs_user(user: dict = Depends(get_current_user)):
        return {"user": user}
...
def test_get_current_user_rejects_invalid_token():
    client = TestClient(_test_app())
    r = client.get("/needs-user", headers={"Authorization": "Bearer invalid-jwt"})
    assert r.status_code == 401
    assert r.json()["detail"]["error"] == "invalid_session"
...
def test_get_current_user_rejects_nonexistent_user():
    client = TestClient(_test_app())
    token = tokens.create_access_token(user_id="507f1f77bcf86cd799439011")
    r = client.get("/needs-user", headers={"Authorization": f"Bearer {token}"})
    assert r.status_code == 401
    assert r.json()["detail"]["error"] == "invalid_session"
    assert "User no longer exists" in r.json()["detail"]["detail"]
